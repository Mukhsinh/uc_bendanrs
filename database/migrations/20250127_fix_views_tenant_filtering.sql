-- Migration: Fix Views for Tenant Filtering
-- Date: 2025-01-27
-- Description: Update views to include tenant_id filtering and ensure RLS works correctly

-- ============================================================================
-- STEP 1: Drop and recreate view_rekapitulasi_unit_cost with tenant_id
-- ============================================================================

DROP VIEW IF EXISTS public.view_rekapitulasi_unit_cost CASCADE;
...
WITH laboratorium AS (
  ...
  WHERE kl.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
radiologi AS (
  SELECT
    kr.id,
  ...
  WHERE kr.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
bdrs AS (
  SELECT
    kb.id,
  ...
  WHERE kb.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
tindakan_inap AS (
  SELECT
    kti.id,
  ...
  WHERE kti.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
tindakan_rawat_jalan AS (
  ...
  WHERE ktrj.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
operatif AS (
  SELECT
    ko.id,
  ...
  WHERE ko.tenant_id = public.get_tenant_id() OR public.is_super_admin()
),
cathlab AS (
  SELECT
    kc.id,
  ...
  WHERE kc.tenant_id = public.get_tenant_id() OR public.is_super_admin()
)
SELECT
  v.id,
  v.user_id,
...
COMMENT ON VIEW public.view_rekapitulasi_unit_cost IS 'View untuk rekapitulasi unit cost dengan tenant isolation';
