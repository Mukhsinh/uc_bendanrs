-- ============================================
-- FIX: Meningkatkan timeout untuk manual_recalculate_bdrs
-- ============================================
-- Masalah: Statement timeout saat rekalkulasi BDRS dengan data besar
-- Solusi: Meningkatkan statement_timeout dari 2 menit (120000ms) ke 10 menit (600000ms)
--         sama seperti yang digunakan untuk Laboratorium
-- ============================================
...
BEGIN
  -- Set timeout for this session - DIPERPANJANG dari 2 menit ke 10 menit
  PERFORM set_config('statement_timeout', '600000', true); -- 10 minutes (600000ms)
  ...
  -- STEP 6: Calculate unit_cost_per_pemeriksaan (FIXED - was missing)
  step_count := step_count + 1;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      RAISE NOTICE 'Warning: Refresh rekapitulasi_unit_cost gagal: %', SQLERRM;
...
-- VERIFICATION
-- ============================================
SELECT 'Fix timeout untuk manual_recalculate_bdrs completed successfully!' as status;
SELECT 'Statement timeout telah ditingkatkan dari 2 menit (120000ms) ke 10 menit (600000ms)' as change;
