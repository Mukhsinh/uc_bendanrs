-- Script untuk membuat tabel kalkulasi_diklat secara manual
...
AND table_name = 'kalkulasi_diklat';
...
-- 4. Buat trigger untuk update timestamp
...
DROP TRIGGER IF EXISTS trigger_update_kalkulasi_diklat_updated_at ON kalkulasi_diklat;
...
-- 5. Buat function untuk menghitung total biaya unit diklat
...
DROP TRIGGER IF EXISTS trigger_calculate_total_biaya_unit_diklat ON kalkulasi_diklat;
...
-- 6. Buat function untuk menghitung biaya diklat per hari
...
DROP TRIGGER IF EXISTS trigger_calculate_biaya_diklat_per_hari ON kalkulasi_diklat;
CREATE TRIGGER trigger_calculate_biaya_diklat_per_hari
    BEFORE INSERT OR UPDATE ON kalkulasi_diklat
    FOR EACH ROW
    EXECUTE FUNCTION calculate_biaya_diklat_per_hari();

-- 7. Buat function untuk menghitung unit cost per jenis layanan
...
DROP TRIGGER IF EXISTS trigger_calculate_unit_cost_per_jenis_layanan ON kalkulasi_diklat;
...
DROP POLICY IF EXISTS "System can update all kalkulasi_diklat" ON kalkulasi_diklat;
DROP POLICY IF EXISTS "System can insert all kalkulasi_diklat" ON kalkulasi_diklat;
DROP POLICY IF EXISTS "System can select all kalkulasi_diklat" ON kalkulasi_diklat;
DROP POLICY IF EXISTS "System can delete all kalkulasi_diklat" ON kalkulasi_diklat;
...
-- 10. Verifikasi tabel sudah dibuat
SELECT 
    table_name,
    column_name,
    data_type,
